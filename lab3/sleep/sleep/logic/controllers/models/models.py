...
class SleepRecord(db.Model):
    ...
    total_awake_time = db.Column(db.Integer
...
class WakeUp(db.Model):
    ...
    wake_up_time = db.Column(db.DateTime, nullable=False)
    reason_id = db.Column(db.Integer, db.ForeignKey('wake_up_reasons.reason_id'), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class WakeUpReason(db.Model):
    ...
    reason_description = db.Column(db.String(255), nullable=False)
